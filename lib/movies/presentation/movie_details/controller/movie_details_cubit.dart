...
import 'package:movie_app_db_example/movies/presentation/movie_details/controller/movie_details_state.dart';
...
class MovieDetailsCubit extends Cubit<MovieDetailsStates> {
  ...
  FutureOr<void> addFavoriteMovies(Movie movie) async {
    ...
    result.fold((l) => emit(InsertMovieToDatabaseErrorState(l.message)),
            (r) {emit(InsertMovieToDatabaseSuccessState());
    });
  ...
  FutureOr<void> deleteFavoriteMovie(int id) async {
    ...
    result.fold(
          (l) => emit(DeleteMovieToDatabaseErrorState(l.message)),
          (r) => emit(DeleteMovieToDatabaseSuccessState()),
    );
  }
  ...
  FutureOr<void> isFavoriteMovie(int id) async {
    ...
    result.fold(
          ...
          (r) => emit(IsFavoriteMovieSuccessState()),
    );
  }
