...
class MoviesRepository extends BaseMoviesRepository {
  ...
  @override
  Future<Either<Failure, void>> addFavoriteMovies(MovieModelDB newMovie) async {
    final result = await baseMovieLocalDataSource.addFavoriteMovies(newMovie);
    try {
      return Right(result);
    } on ServerException catch (failure) {
  ...
  Future<Either<Failure, List<MovieModelDB>>> getAllFavoriteMovies() async {
    final result = await baseMovieLocalDataSource.getAllFavoritesMovies();
    try {
      return Right(result);
    } on ServerException catch (failure) {
      return Left(DatabaseFailure(failure.errorMessageModel.statusMessage));
